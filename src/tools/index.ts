        ...
        async ({ owner, name }) => {
            try {
                ...
                const languages = repo.languages.edges.map((edge) =>
                    `${edge.node.name} (${(edge.size / 1024).toFixed(1)} KB)`
                ).join(', ');
...
`;
    ...
    server.tool(
        ...
        "Search for files in a GitHub repository",
        {
            owner: z.string().describe("Repository owner (username or organization)"),
            name: z.string().describe("Repository name"),
            path: z.string().optional().describe("Directory path within repository (optional)"),
            query: z.string().optional().describe("Search term to filter files (optional)")
        },
    ...
    server.tool(
        ...
        async ({ owner, name, path }) => {
            try {
                ...
                const codeLanguages = ['js', 'ts', 'jsx', 'tsx', 'py', 'java', 'c', 'cpp', 'cs', 'go', 'rb', 'php', 'rust', 'scala', 'swift', 'kt'];
    ...
    server.tool(
        ...
        "Get the directory structure of a GitHub repository",
        {
            ...
            name: z.string().describe("Repository name"),
            path: z.string().optional().describe("Directory path within repository (optional)"),
            maxDepth: z.number().min(1).max(5).default(3).describe("Maximum recursion depth (1-5)")
        },
        async ({ owner, name, path = "", maxDepth = 3 }) => {
            try {
                const structure = await github.getRepositoryStructure(owner, name, path, maxDepth);

                // Recursively format the structure
                function formatStructure(item: github.StructureEntry, level = 0): string {
    ...
    server.tool(
        ...
        async ({ owner, name, query }) => {
            try {
                ...
                const filesFound = results.items.map((item: CodeSearchResultItem) =>
