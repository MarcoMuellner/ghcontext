...
describe("File Search", () => {
  ...
  describe("searchFiles", () => {
    ...
    it("should handle single file response from getContent", async () => {
      ...
      expect(cache.get).toHaveBeenCalledWith(
        "files:facebook/react:package.json:",
      );
      expect(mockRestClient.repos.getContent).toHaveBeenCalledWith({
        owner: "facebook",
        repo: "react",
    ...
    it("should throw error when path is not found", async () => {
      ...
      // Act & Assert
      await expect(
        searchFiles("facebook", "react", "nonexistent"),
    ...
    it("should throw error for other API errors", async () => {
      ...
      // Act & Assert
      await expect(searchFiles("facebook", "react", "src")).rejects.toThrow(
        "GitHub API error",
      );
      expect(cache.set).not.toHaveBeenCalled(); // Don't cache errors
    });
  ...
  describe("searchRepositoryCode", () => {
    it("should return cached search results if available", async () => {
        ...
        items: [
          {
            name: "file.js",
            path: "src/file.js",
            html_url: "https://github.com/file.js",
          },
        ],
      };
      ...
      // Assert
      expect(cache.get).toHaveBeenCalledWith(
        "code-search:facebook/react:useState:30",
      );
        ...
        data: {
          items: [
            ...
            },
            {
              name: "other.js",
              path: "src/other.js",
              html_url: "https://github.com/other.js",
            },
          ],
        },
      });
    ...
    it("should handle API errors", async () => {
      // Arrange
      vi.mocked(cache.get).mockReturnValue(undefined); // Cache miss
      mockRestClient.search.code.mockRejectedValue(
        new Error("API rate limit exceeded"),
      );

      // Act & Assert
      await expect(
        searchRepositoryCode("facebook", "react", "useState"),
