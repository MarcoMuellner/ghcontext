 ...
 * @private
 */
async function findDedicatedApiDocs(owner: string, name: string): Promise<string | null> {
    ...
    try {
        ...
        const docsPaths = files.filter(path =>
            path.includes('/docs/') ||
...
function extractCodeBlocks(readme: string): string | null {
    ...
    const apiExamples = codeBlocks.filter(block => {
        ...
        return /\w+\(.*\)/.test(block) ||       // function calls
            /\w+\.\w+/.test(block) ||        // method invocations
            /import\s+.*from\s+/.test(block); // import statements
    });
