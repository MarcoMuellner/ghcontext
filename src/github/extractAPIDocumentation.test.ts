...
const readmeWithoutApiSection = `# Sample Repository
...
const apiMdContent = `# API Reference
...
describe('API Documentation Extraction', () => {
    ...
    it('should extract API sections from README when not cached', async () => {
        ...
        vi.mocked(getReadmeContentModule.getReadmeContent).mockResolvedValue(readmeWithApiSection);
        ...
        // Assert
        expect(cacheModule.get).toHaveBeenCalledWith('api-docs:facebook/react');
        expect(getReadmeContentModule.getReadmeContent).toHaveBeenCalledWith('facebook', 'react');
    ...
    it('should extract code blocks as examples when no API sections or docs found', async () => {
        // Arrange
        vi.mocked(cacheModule.get).mockReturnValue(undefined);
        vi.mocked(getReadmeContentModule.getReadmeContent).mockResolvedValue(readmeWithCodeBlocks);

        // All file content lookups fail
        vi.mocked(getFileContentModule.getFileContent).mockRejectedValue(new Error('File not found'));
        vi.mocked(getRepositoryStructureModule.getRepositoryFiles).mockResolvedValue([]);
    ...
    it('should return null when README not found', async () => {
        ...
        expect(cacheModule.set).toHaveBeenCalledWith('api-docs:facebook/react', null);
    });

    it('should handle errors gracefully', async () => {
        // Arrange
        vi.mocked(cacheModule.get).mockReturnValue(undefined);
        vi.mocked(getReadmeContentModule.getReadmeContent).mockRejectedValue(new Error('API rate limit exceeded'));
